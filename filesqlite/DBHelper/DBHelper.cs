...
namespace mvvmlight.Database
{
    public class SqLiteRepository : IRepository
    {
        ...
        public List<T> GetList<T, TU>(string para, TU val, int top = 0) where T : class, new()
        {
            ...
            var list = this.connection.Query<T>(sql, val);
            if (list.Count != 0)
            {
                if (top != 0)
                {
        ...
        void CreateTables()
        {
            connection.CreateTable<LoginData>();
            connection.CreateTable<JourneyData>();
            connection.CreateTable<UserDetailsData>();
            connection.CreateTable<PricesData>();
        }
